...
class Simulator:
    ...
    @classmethod
    def from_config(cls, config, quiet=False):
        """Open a simulation database from a validated storage config."""
        if not isinstance(config, StorageConfig):
            raise TypeError("config must be a simcat.config.StorageConfig")
        return cls(config.name, config.workdir, quiet=quiet)
    ...
    def run(
        ...
        if recover:
            self.recover()
        # ipyparallel is an optional HPC dependency. Keep it out of the import
        # path until the distributed backend is actually requested.
